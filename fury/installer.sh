...
# لو ملقاش python3، يجرب أمر python العادي
...
# 5. تحميل وتثبيت DataMonitor بناءً على إصدار البايثون
if [ -n "$PYTHON_VERSION" ]; then
    ...
    if grep -q "Not Found" /tmp/datamonitor.ipk || [ ! -s /tmp/datamonitor.ipk ]; then
        ...
        opkg install --force-overwrite /tmp/datamonitor.ipk
        rm -f /tmp/datamonitor.ipk
    fi
fi
sleep 1
...
# 6. تحميل وتثبيت FuryDisk بناءً على إصدار البايثون
if [ -n "$PYTHON_VERSION" ]; then
